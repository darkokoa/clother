...
# --- ASCII Art Banner ---
# Fixed: Using read with heredoc to properly handle special characters
...
# --- Provider Launchers ---
# Fixed: Using proper heredoc with escaped delimiter to handle special characters
cat > "$BIN/clother-native" <<'NATIVEEOF'
#!/usr/bin/env bash
set -euo pipefail
IFS=$'\n\t'
cat <<'BANNER'
